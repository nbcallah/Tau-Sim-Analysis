...
bool absorb(double ePerp, double u, double contam) {
//    printf("%f\n", contam);
    double v = 2*M_PI*HBAR*HBAR*(NCARBON*contam)*ACARBON/MASS_N + 2*M_PI*HBAR*HBAR*NBORON*ABORON/MASS_N;
    double w = HBAR*NBORON*SIGMABORON/2;
...
std::vector<weightedBin> createHistQuant(double contam, double threshold, double saturation, std::vector<evt>& events, double* randU01s, double* randDeathTimes) {
//void createHist(double absProb, double absCut, double threshold, double saturation, std::vector<evt>& events, double* randU01s) {
    ...
    for(unsigned long i = 0; i < events.size(); i++) {
        ...
        for(int j = 0; j < 50; j++) {
            ...
            if(absorb(events[i].ePerp[j], randU01s[i*100 + j], contam)) {
    ...
    for(int i = 0; i < nBins+1; i++) {
        for(int j = 0; j < nBins+1; j++) {
            for(int k = 0; k < nBins+1; k++) {
                double thresh = 2.0+4.0*i/(double)nBins;
                double sat = 24+10.5*j/(double)nBins;
                double contam = 1.0/(2+5*k/(double)nBins);
