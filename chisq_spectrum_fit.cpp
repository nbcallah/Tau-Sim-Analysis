...
//std::vector<double> createHist(double absProb, double absCut, double threshold, double saturation, std::vector<evt>& events) {
//    std::vector<double> hist;
//    hist.resize(185, 0.0);
//    for(auto it = events.begin(); it < events.end(); it++) {
//        double weight = 0.0;
//        if(it->times < 40 || it->times > 225) {
//            continue;
//        }
...
int main(int argc, char** argv) {
    ...
    for(auto it = events.begin(); it < events.end(); it++) {
//        printf("%e %e %e %e\n", it->times[0], it->times[49], it->ePerp[0], it->ePerp[49]);
    }
